...
UA = {"User-Agent": "Cloudflare-Service-Radar/1.0 (+github-actions)"}
REQUEST_TIMEOUT = 20
CACHE_TTL_SECONDS = 120

CLOUDFLARE_STATUS_PAGE_URL = "https://www.cloudflarestatus.com/"
CLOUDFLARE_STATUS_API_STATUS_URL = "https://www.cloudflarestatus.com/api/v2/status.json"
CLOUDFLARE_STATUS_API_COMPONENTS_URL = "https://www.cloudflarestatus.com/api/v2/components.json"
CLOUDFLARE_STATUS_API_INCIDENTS_URL = "https://www.cloudflarestatus.com/api/v2/incidents.json"
CLOUDFLARE_STATUS_HISTORY_RSS_URL = "https://www.cloudflarestatus.com/history.rss"

STATUSGATOR_URL = "https://statusgator.com/services/cloudflare"
ISDOWN_STATUS_URL = "https://isdown.app/status/cloudflare"

STATUSPAGE_API_DOCS_URL = "https://support.atlassian.com/statuspage/docs/what-are-the-different-apis-under-statuspage/"
CLOUDFLARE_STATUS_API_DOCS_URL = "https://www.cloudflarestatus.com/api"
CLOUDFLARE_HELP_URL = "https://developers.cloudflare.com/support/"
...
def _build_cloudflare_official_summary(
    description: str,
    current_status: str,
    ...
    if impactful_active_incidents:
        ...
        latest_title = _clean(latest.get("title")) or "Cloudflare incident"
        if len(impactful_active_incidents) == 1:
            return f"Cloudflare Statuspage reports an active incident: {latest_title}."
        return f"Cloudflare Statuspage reports {len(impactful_active_incidents)} active incidents. Latest: {latest_title}."

    normalized_status = _normalize_outage_status_text(current_status)
    if normalized_status == "operational" and description:
        return f"Cloudflare Statuspage reports {description}. No active incidents are listed."
    ...
    if description and isinstance(latest_age_h, float):
        ...
        if latest_age_h <= 24:
            return f"Cloudflare Statuspage reports {description}. Latest listed incident started about {rounded}h ago."
        return f"Cloudflare Statuspage reports {description}. Latest listed incident was about {rounded}h ago."
    if description:
        return f"Cloudflare Statuspage reports {description}."
    if recent_incidents:
        return f"Cloudflare Statuspage incident history is available. Latest listed incident: {recent_incidents[0].get('title')}."
    return "Official Cloudflare status information is currently unavailable."


def fetch_cloudflare_statuspage_bundle() -> dict[str, Any]:
    checked_at = _utc_now_iso()
    status_payload = _request_json(CLOUDFLARE_STATUS_API_STATUS_URL)
    components_payload = _request_json(CLOUDFLARE_STATUS_API_COMPONENTS_URL)
    incidents_payload = _request_json(CLOUDFLARE_STATUS_API_INCIDENTS_URL)
    return parse_statuspage_official_payloads(
        status_payload=status_payload,
        components_payload=components_payload,
        incidents_payload=incidents_payload,
        page_url=CLOUDFLARE_STATUS_PAGE_URL,
        source_name="Cloudflare Statuspage API",
        summary_builder=_build_cloudflare_official_summary,
...
def _synthesize_statusgator_summary(
    ...
    if normalized_status != "unknown" and isinstance(reports_24h, int):
        return (
            f"StatusGator indicates Cloudflare is currently {normalized_status} "
    ...
    if normalized_status != "unknown":
        ...
        return f"StatusGator indicates Cloudflare is currently {normalized_status}."
    if incidents:
...
def _extract_isdown_status_text(page_text: str) -> tuple[str, str]:
    ...
    summary = f"IsDown indicates Cloudflare is {status_phrase}."
...
def _build_official_block(official_updates: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for item in _sort_by_datetime(official_updates, field="published_at"):
        ...
        updates.append(
            {
                "title": _clean(item.get("title")) or "Cloudflare status update",
                "url": url,
                "published_at": item.get("published_at"),
                "source": _clean(item.get("source")) or "Cloudflare Statuspage API",
    ...
    updates = updates[:10]
    return {
        "summary": updates[0].get("title") if updates else "Official Cloudflare status updates unavailable.",
        "updates": updates,
...
def _collect_payload(scoring_profile: str | None = None) -> dict[str, Any]:
    ...
    statusgator_run = _run_cloudflare_source(
    ...
    isdown_run = _run_cloudflare_source(
        adapter_id="isdown_cloudflare",
        name="IsDown (Cloudflare)",
    ...
    official_run = _run_cloudflare_source(
        adapter_id="cloudflare_statuspage_api",
        name="Cloudflare Statuspage API",
        kind="official-api",
        url=CLOUDFLARE_STATUS_API_STATUS_URL,
        role="official",
        criticality="required",
        used_for_scoring=True,
        fetch_fn=fetch_cloudflare_statuspage_bundle,
    ...
    if official_status:
        ...
        if official_status_text != "unknown":
            ...
            outage["current_status_origin"] = "Cloudflare Statuspage API"
        if official_status.get("summary"):
            outage["summary"] = official_status.get("summary")
            outage["summary_origin"] = "Cloudflare Statuspage API"
        outage["url"] = CLOUDFLARE_STATUS_PAGE_URL
        ...
        if isinstance(top_component_issues, list) and top_component_issues:
            ...
            outage["top_reported_issues_meta"] = {
                "source": "Cloudflare Statuspage API",
    ...
    analytics["model_version"] = "cloudflare-1.0"
    ...
    known_resources = [
        {
            ...
            "url": CLOUDFLARE_STATUS_PAGE_URL,
            "source": "Official",
            "meta": "Statuspage",
            "published_at": generated_at,
        },
        {
            "title": "Cloudflare Status API (Statuspage v2)",
            "url": CLOUDFLARE_STATUS_API_STATUS_URL,
            "source": "Official",
            "meta": "JSON endpoint",
            "published_at": generated_at,
        ...
        {
            "title": "Cloudflare Status history RSS",
            "url": CLOUDFLARE_STATUS_HISTORY_RSS_URL,
            "source": "Official",
            "meta": "RSS feed",
            "published_at": generated_at,
        },
        {
            "title": "Cloudflare Status API docs",
            "url": CLOUDFLARE_STATUS_API_DOCS_URL,
            "source": "Official",
