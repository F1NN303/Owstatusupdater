...
UA = {"User-Agent": "Discord-Service-Radar/1.0 (+github-actions)"}
REQUEST_TIMEOUT = 20
CACHE_TTL_SECONDS = 120

DISCORD_STATUS_PAGE_URL = "https://discordstatus.com/"
DISCORD_STATUS_API_STATUS_URL = "https://discordstatus.com/api/v2/status.json"
DISCORD_STATUS_API_COMPONENTS_URL = "https://discordstatus.com/api/v2/components.json"
DISCORD_STATUS_API_INCIDENTS_URL = "https://discordstatus.com/api/v2/incidents.json"
DISCORD_STATUS_HISTORY_RSS_URL = "https://discordstatus.com/history.rss"

STATUSGATOR_URL = "https://statusgator.com/services/discord"
ISDOWN_STATUS_URL = "https://isdown.app/status/discord"

STATUSPAGE_API_DOCS_URL = "https://support.atlassian.com/statuspage/docs/what-are-the-different-apis-under-statuspage/"
DISCORD_HELP_STATUS_URL = "https://support.discord.com/hc/en-us"
...
def _build_discord_official_summary(
    description: str,
    current_status: str,
    ...
    if impactful_active_incidents:
        ...
        latest_title = _clean(latest.get("title")) or "Discord incident"
        if len(impactful_active_incidents) == 1:
            return f"Discord Statuspage reports an active incident: {latest_title}."
        return f"Discord Statuspage reports {len(impactful_active_incidents)} active incidents. Latest: {latest_title}."

    normalized_status = _normalize_outage_status_text(current_status)
    if normalized_status == "operational" and description:
        return f"Discord Statuspage reports {description}. No active incidents are listed."
    ...
    if description and isinstance(latest_age_h, float):
        ...
        if latest_age_h <= 24:
            return f"Discord Statuspage reports {description}. Latest listed incident started about {rounded}h ago."
        return f"Discord Statuspage reports {description}. Latest listed incident was about {rounded}h ago."
    if description:
        return f"Discord Statuspage reports {description}."
    if recent_incidents:
        return f"Discord Statuspage incident history is available. Latest listed incident: {recent_incidents[0].get('title')}."
    return "Official Discord status information is currently unavailable."


def fetch_discord_statuspage_bundle() -> dict[str, Any]:
    checked_at = _utc_now_iso()
    status_payload = _request_json(DISCORD_STATUS_API_STATUS_URL)
    components_payload = _request_json(DISCORD_STATUS_API_COMPONENTS_URL)
    incidents_payload = _request_json(DISCORD_STATUS_API_INCIDENTS_URL)
    return parse_statuspage_official_payloads(
        status_payload=status_payload,
        components_payload=components_payload,
        incidents_payload=incidents_payload,
        page_url=DISCORD_STATUS_PAGE_URL,
        source_name="Discord Statuspage API",
        summary_builder=_build_discord_official_summary,
...
def _synthesize_statusgator_summary(
    ...
    if normalized_status != "unknown" and isinstance(reports_24h, int):
        return (
            f"StatusGator indicates Discord is currently {normalized_status} "
    ...
    if normalized_status != "unknown":
        ...
        return f"StatusGator indicates Discord is currently {normalized_status}."
    if incidents:
...
def _extract_isdown_status_text(page_text: str) -> tuple[str, str]:
    ...
    summary = f"IsDown indicates Discord is {status_phrase}."
...
def _build_official_block(official_updates: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for item in _sort_by_datetime(official_updates, field="published_at"):
        ...
        updates.append(
            {
                "title": _clean(item.get("title")) or "Discord status update",
                "url": url,
                "published_at": item.get("published_at"),
                "source": _clean(item.get("source")) or "Discord Statuspage API",
    ...
    updates = updates[:10]
    return {
        "summary": updates[0].get("title") if updates else "Official Discord status updates unavailable.",
        "updates": updates,
...
def _collect_payload(scoring_profile: str | None = None) -> dict[str, Any]:
    ...
    statusgator_run = _run_discord_source(
    ...
    isdown_run = _run_discord_source(
        adapter_id="isdown_discord",
        name="IsDown (Discord)",
    ...
    official_run = _run_discord_source(
        adapter_id="discord_statuspage_api",
        name="Discord Statuspage API",
        kind="official-api",
        url=DISCORD_STATUS_API_STATUS_URL,
        role="official",
        criticality="required",
        used_for_scoring=True,
        fetch_fn=fetch_discord_statuspage_bundle,
    ...
    if official_status:
        ...
        if official_status_text != "unknown":
            ...
            outage["current_status_origin"] = "Discord Statuspage API"
        if official_status.get("summary"):
            outage["summary"] = official_status.get("summary")
            outage["summary_origin"] = "Discord Statuspage API"
        outage["url"] = DISCORD_STATUS_PAGE_URL
    ...
    analytics["model_version"] = "discord-1.0"
    ...
    known_resources = [
        {
            ...
            "url": DISCORD_STATUS_PAGE_URL,
            "source": "Official",
            "meta": "Statuspage",
            "published_at": generated_at,
        },
        {
            "title": "Discord Status API (Statuspage v2)",
            "url": DISCORD_STATUS_API_STATUS_URL,
            "source": "Official",
            "meta": "JSON endpoint",
            "published_at": generated_at,
        ...
        {
            "title": "Discord Status history RSS",
            "url": DISCORD_STATUS_HISTORY_RSS_URL,
            "source": "Official",
            "meta": "RSS feed",
            "published_at": generated_at,
        },
