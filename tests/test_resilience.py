...
import services.epic_aggregator as epic_aggregator
...
class EpicAggregatorResilienceTests(unittest.TestCase):
    def test_collect_payload_with_partial_source_failures(self) -> None:
        statusgator_data = {
            "source": "StatusGator",
            "source_type": "Downdetector-like",
            "url": "https://statusgator.com/services/epic-games",
            "summary": "StatusGator indicates Epic Games is currently degraded.",
            "current_status": "degraded",
            "reports_24h": 38,
            "incidents": [
                {
            ...
            "top_reported_issues": [{"label": "Game Services", "count": 3}],
        }
        ...
        def _run_side_effect(**kwargs):
            ...
            if adapter_id == "isdown_epic_games":
                return SourceRunResult(
                    ok=False,
                    data=None,
                    source=_source_entry("IsDown (Epic Games)", False),
                    error="simulated failure",
                )
            if adapter_id == "epic_games_statuspage_api":
                return SourceRunResult(
                    ok=False,
                    data=None,
                    source=_source_entry("Epic Games Statuspage API", False),
                    error="simulated failure",
                )
            raise AssertionError(f"Unexpected adapter_id: {adapter_id}")

        with patch("services.epic_aggregator._run_epic_source", side_effect=_run_side_effect):
            payload = epic_aggregator._collect_payload(scoring_profile="official_first_v1")
...
class SlackAggregatorResilienceTests(unittest.TestCase):
    def test_collect_payload_with_partial_source_failures(self) -> None:
        statusgator_data = {
            "source": "StatusGator",
            "source_type": "Downdetector-like",
            "url": "https://statusgator.com/services/slack",
            "summary": "StatusGator indicates Slack is currently degraded.",
            "current_status": "degraded",
            "reports_24h": 57,
            "incidents": [
                {
            ...
            "top_reported_issues": [{"label": "Notifications", "count": 4}],
        }
        ...
        def _run_side_effect(**kwargs):
            ...
            if adapter_id == "isdown_slack":
                return SourceRunResult(
                    ok=False,
                    data=None,
                    source=_source_entry("IsDown (Slack)", False),
                    error="simulated failure",
                )
            if adapter_id == "slack_status_api":
                return SourceRunResult(
                    ok=False,
                    data=None,
                    source=_source_entry("Slack Status API", False),
                    error="simulated failure",
                )
            raise AssertionError(f"Unexpected adapter_id: {adapter_id}")
...
class XAggregatorResilienceTests(unittest.TestCase):
    def test_collect_payload_with_partial_source_failures(self) -> None:
        statusgator_data = {
            "source": "StatusGator",
            "source_type": "Downdetector-like",
            "url": "https://statusgator.com/services/x",
            "summary": "StatusGator indicates X is currently degraded.",
            "current_status": "degraded",
            "reports_24h": 57,
            "incidents": [
                {
            ...
            "top_reported_issues": [{"label": "Posting", "count": 6}],
        }
        ...
        def _run_side_effect(**kwargs):
            ...
            if adapter_id == "isdown_x":
                return SourceRunResult(
                    ok=False,
                    data=None,
                    source=_source_entry("IsDown (X)", False),
                    error="simulated failure",
                )
            if adapter_id == "x_developer_status":
                return SourceRunResult(
                    ok=False,
                    data=None,
                    source=_source_entry("X Developer Platform Status", False),
                    error="simulated failure",
                )
            raise AssertionError(f"Unexpected adapter_id: {adapter_id}")

        with patch("services.x_aggregator._run_x_source", side_effect=_run_side_effect):
            payload = x_aggregator._collect_payload(scoring_profile="official_first_v1")
